...
api.interceptors.request.use((config) => {
  if (typeof window !== 'undefined') {
    ...
    if (token && (config.url?.startsWith('/admin') || config.url?.startsWith('/plz-surcharges') || config.method === 'put' || config.method === 'delete')) {
...
export const trackingApi = {
  ...
  postLocation: async (booking_number: string, lat: number, lng: number, token: string) => {
    ...
    return response.data as { ok: boolean; driver_status: string; pickup: { lat: number; lng: number } | null; pickup_address: string | null };
  },
};
