        ...
        {city.highlight && city.highlight.length > 0 && (
          <section className="mb-10">
            <h2 className="text-2xl font-bold text-gray-900 mb-6">
              {isEN ? 'Top Attractions Nearby' : 'Highlights & Top-Ausflugsziele'}
            </h2>
            <div className="space-y-6">
              {city.highlight.map((h, i) => (
                <div key={i} className="rounded-2xl border border-yellow-200 bg-gradient-to-br from-yellow-50 to-white p-6 shadow-sm">
                  <div className="flex items-start gap-3 mb-3">
                    <span className="mt-0.5 text-yellow-500 flex-shrink-0">
                      <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 20 20">
                        <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                      </svg>
                    </span>
                    <h3 className="text-lg font-bold text-gray-900 leading-tight">
                      {isEN ? h.titleEN : h.titleDE}
                    </h3>
                  </div>
                  <p className="text-gray-700 leading-relaxed text-sm">
                    {isEN ? h.textEN : h.textDE}
                  </p>
                </div>
              ))}
            </div>
